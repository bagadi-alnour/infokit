 ...
 * behind /dashboard/my-organization/skills.
 *
...
function refresh(locale: Locale) {
  revalidatePath(localizedPath("/dashboard/my-organization/skills", locale));
  // The member profile picks its declarations from this catalogue.
  revalidatePath(localizedPath("/dashboard/my-organization/city-team", locale));
}

function notice(locale: Locale, code: string): never {
  redirect(
    `${localizedPath("/dashboard/my-organization/skills", locale)}?notice=${code}`,
  );
}
