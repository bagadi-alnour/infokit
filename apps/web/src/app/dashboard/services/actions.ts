...
export async function upsertServiceTranslation(formData: FormData) {
  ...
  await db
    ...
    .values(parsed)
    .onConflictDoUpdate({
      target: [serviceTranslations.serviceId, serviceTranslations.languageCode],
      set: {
        name: parsed.name,
