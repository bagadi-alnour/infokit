/**
 * The Amazon RDS root certificate authorities for eu-west-3, used to prove that the
 * endpoint answering for our database really is our database.
 *
 * This is a public certificate bundle, not a secret. It is committed rather than
 * fetched so a deploy never depends on reaching AWS at build time, and inlined as a
 * string rather than shipped as a `.pem` because a file read would need
 * `outputFileTracingIncludes` to survive Vercel's bundling — and a CA that goes
 * missing fails closed at connect time, in production only, which is the worst place
 * to discover a packaging mistake.
 *
 * Fetched 2026-07-29 from the official truststore, which is the only place to
 * refresh it from:
 *   https://truststore.pki.rds.amazonaws.com/eu-west-3/eu-west-3-bundle.pem
 *
 * Three roots, so a server presenting any of the three key types verifies:
 *   Amazon RDS eu-west-3 Root CA ECC384 G1    expires 2121-05-25
 *   Amazon RDS eu-west-3 Root CA RSA2048 G1   expires 2061-05-25
 *   Amazon RDS eu-west-3 Root CA RSA4096 G1   expires 2121-05-25
 *
 * Nothing here expires this century, so rotation is not a scheduled chore. It does
 * need replacing if the database ever moves to another region — the bundle is
 * region-specific, and a mismatched one fails the handshake rather than degrading
 * quietly, which is the behaviour we want.
 */
export const RDS_CA_EU_WEST_3 = `
-----BEGIN CERTIFICATE-----

SuwWVLhu2wWDuSW+K7tTuL8p
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;
