...
function SelectContent({
  ...
  return (
    <SelectPrimitive.Portal>
      ...
      >
        {/* At least as wide as the field, but free to grow to its longest
         * option: an option is a choice, and a choice you cannot read — “Soutien
         * psychologiqu…” — cannot be made. The available width keeps the popup
         * inside the viewport, and a select that must match its field exactly
         * passes its own `w-*`. */}
        <SelectPrimitive.Popup
          data-slot="select-content"
          data-align-trigger={alignItemWithTrigger}
          className={cn(
            "max-h-(--available-height) max-w-(--available-width) origin-(--transform-origin) bg-popover text-popover-foreground ring-foreground/10 data-[side=bottom]:slide-in-from-top-2 data-[side=inline-end]:slide-in-from-start-2 data-[side=inline-start]:slide-in-from-end-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 relative isolate z-50 w-max min-w-[max(9rem,var(--anchor-width))] overflow-y-auto overflow-x-hidden rounded-lg shadow-md ring-1 duration-100 data-[align-trigger=true]:animate-none",
            className,
          )}
          {...props}
        >
