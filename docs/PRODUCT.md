...
| Activity                    | One visitor-facing recurring or scheduled offering coordinated by one organisation in one city, or held by the platform itself — provisionally before an organisation is known/onboarded, or as the platform's own record when no organisation stands behind it. One or more organisations may create, provide, and verify it; it owns its place, audience, schedule, occurrence status, contact, and freshness |
...
| Factual owner               | The organisation responsible for the accuracy of public information, including when the platform publishes on its behalf; the platform itself where no organisation stands behind the record                                                                                                                                                                                                                    |
...
| FR-P1-004 | An activity shows its reusable services, place/directions, schedule, current status (including the next opening time when closed), each providing association's approved logo and name where one is named, safe contact, language/accessibility data, and last verification                                                                                                                                                                                                               | P0       |
...
| FR-P1-033 | Every published activity and public event either has at least one verified providing association whose approved logo and text name remain visible on cards, details, and map results, or is published by the platform itself when no association stands behind it — in which case no association is named anywhere and the provider line is omitted rather than left empty                                                                                                                | P0       |
...
| FR-P1-051 | When creating an activity, a platform editor chooses its owner: an association, or the platform itself. A platform-owned activity records no coordinating organisation, no creator, and no provider; it publishes on the platform's own responsibility and never names an association as its provider. Choosing an association keeps the ordinary provider relationship and its publication gate                                                                                          | P0       |
...
- Every published activity/event has one audience category and either at least one verified provider with approved logo and visible text name, or the platform as its publisher and no named provider.
...
1. A platform editor links a known organisation provisionally, or leaves the coordinating organisation empty only when no factual provider is known — including the case where the platform keeps the record as its own and no organisation is ever expected to claim it.
...
- Publication requires at least one confirmed, verified provider, or the platform as publisher. The platform is the publisher only when no organisation stands behind the activity: the record then names no custodian, no creator, and no provider, and the platform answers for its accuracy and freshness itself. When the organisation behind the activity is known, it is linked and confirms the relationship rather than being represented by a null organisation merely because the platform entered the record.
- Temporary launch allowance: until an organisation's representative can accept a proposed relationship inside the product, a platform editor publishing on a linked organisation's behalf confirms that provider relationship, and the confirmation records the platform editor as the acting confirmer. This allowance is removed once acceptance exists, and it never replaces a concerned organisation's required approval for public attribution.
...
- Public information never becomes platform-owned merely because a platform editor entered it: the platform holds a record only where no organisation stands behind it, and a known organisation is linked instead.
...
- An activity has one coordinating custodian for workspace/RLS and city-team routing. `null` custody is permitted for provisional intake when no organisation is known, and for an activity the platform publishes as its own record; it is not a shortcut for platform-created records on behalf of a known organisation.
- Platform provisioning records the real initiating actor and platform scope while preserving every proposed creator/provider organisation. The platform is not inserted as a factual provider solely because it created the row: a platform-published activity has no provider row, and the platform's responsibility is read from its `null` custody and platform creation scope.
...
| Provider integrity          | Published service/events that name a provider organisation without its verified logo and text name; platform-published records name no provider at all              | Zero                                                       |
...
- Every published activity/event names at least one verified provider and shows each provider's approved logo with its text name, unless the platform is its publisher — a platform-held record names no provider rather than naming nobody.
- Activities retain one coordinating custodian while supporting several creator, provider, and verifier organisations. Platform-only custody is either provisional intake for an unknown or not-yet-onboarded organisation, or the platform holding its own record; it is never a substitute for a known organisation's factual ownership.
...
- `DECIDED (platform as publisher, 27 July 2026)` — An activity no organisation stands behind is held and published by the platform itself: no custodian organisation, no creator or provider relationship, and the publication gate reads "at least one confirmed, verified provider, or the platform as publisher". Editors choose the platform explicitly in the owner list, and the public surfaces omit the provider line instead of naming nobody.
- `PARTIAL (approval evidence for proxy publication, 27 July 2026)` — Until an organisation's representative can accept a proposed relationship inside the product, a platform editor publishing on a linked organisation's behalf confirms that provider relationship, recorded as confirmed by that editor. The evidence a representative must supply, and the removal of this temporary allowance, remain `TBD`.
...
- `DECIDED (database and application hosting, 29 July 2026)` — One PostgreSQL 18.4 database on Amazon RDS in `eu-west-3`, with the Next.js application on Vercel. The database and the private asset bucket live in a **dedicated AWS account** created for InfoKit (`742338855563`), separate from the operator's earlier projects; email and SMS do not yet, which is its own dated interim below. The master username is `postgres`, which is not cosmetic — `ALTER DEFAULT PRIVILEGES FOR ROLE postgres` in `apps/web/drizzle/0002_roles.sql` silently does nothing under any other name. The application connects as `infokit_app`, never the owner, over TLS verified against the region's RDS certificate bundle (`verify-full`: chain **and** hostname, because a non-expiring password on an unauthenticated channel means one successful interception yields a credential that keeps working). Two connections per serverless instance against `CONNECTION LIMIT 90` — Vercel scales horizontally, so the multiplier is the instance count, not the pool size. Migrations are applied from the operator's machine (`ENV_FILE=… pnpm db:migrate:remote`), so the deployment never holds a credential that can drop its own schema, and the demo-content seed must never be reachable from it: `SEED_DEMO_FORCE=1` belongs in no environment, least of all this one (`AGENTS.md` rule 5). Note for whoever runs the first deploy: the build prerenders 39 routes that read published content, so it needs database reach — a build with no route to the database fails rather than shipping empty pages, which is the correct failure but a surprising one. The schema and catalogue went in on 29 July 2026 — 171 tables, the taxonomy seeds, and a bootstrap superadmin deliberately created with no password hash, so the first credential is chosen through the reset flow rather than existing in a file. Three defects surfaced in doing it, all recorded in `ENGINEERING-NOTES.md` §4, and all variations on one theme: a CLI aimed at a remote database that fails in a way pointing somewhere other than its cause.
- `TBD (dated interim, 29 July 2026)` — The RDS instance is **publicly reachable, and its security group allows `0.0.0.0/0` on 5432** — measured, not assumed. An earlier draft of this entry said "restricted by security group", which was aspiration: Vercel's egress addresses are not fixed on a standard plan, so there is no CIDR to narrow to without static egress or a proxy, and RDS exposes no `pg_hba.conf` in which to restrict the master user by address. What actually defends the instance is therefore three things, all verified: `rds.force_ssl` is on, so a plaintext connection is refused outright rather than merely discouraged; the master password is long and random and is rotated rather than reused; and `infokit_app` — the only credential the deployment holds — owns nothing and has no `UPDATE` or `DELETE` on the append-only tables, so the internet-facing credential is not the one that can drop the schema. The successor is a private subnet with either RDS Proxy or a bastion, and the trigger is the first of: real personal data in the database, a second operator, or an availability requirement that justifies the extra moving parts. Recorded plainly because "public endpoint" is the kind of interim that quietly becomes permanent.
- `TBD (dated interim, 29 July 2026)` — **Email and SMS send from the operator's older AWS account (`211125336353`), not InfoKit's.** Leaving a sending sandbox is granted per account and is not inherited from an AWS Organization, so the new account starts unable to reach unverified recipients while the old one is already out of both the SES and the SNS SMS sandbox with `echoparole.com` verified. Only one consequence is visible to users, and it is the SMS one. Email was expected to be the visible half and is not: `infokit.org` was verified as a domain identity **inside the borrowed account** on 29 July 2026, with Easy DKIM written directly into its Route 53 zone — which happens to live in that same account — so sign-in mail says `contact@infokit.org` even though it leaves `211125336353`. What remains visible is that SMS codes arrive branded `ECHOPAROLE`, and the reason is narrower than "registration": France accepts unregistered sender IDs, so nothing is pending approval. The binding constraint is that a sender ID is set per message or else falls back to the sending account's `DefaultSenderID`, which that account has set to `ECHOPAROLE` for its own project — and France supports no long codes, so there is no number to fall back to instead. An InfoKit-specific sender ID passed per message is therefore the only lever that does not alter the other project's behaviour, and `INFOKIT` was chosen on 29 July 2026 over the alternatives — clearing the shared account default would have unbranded the other project's SMS as a side effect, and a dedicated French number cannot be bought at all. Worth recording because it inverts the intuition: `AWS_SNS_SENDER_ID` looks optional and is the opposite, since unsetting it is precisely how InfoKit's codes inherit the other project's branding.

  Both halves were tested live on 29 July 2026 against a real French handset and inbox, through the scoped `infokit-messaging` identity rather than an administrator's. SNS reported `SUCCESS` from carrier SFR with a device acknowledgement, and SES accepted the message from `contact@infokit.org` with the account `HEALTHY` and the recipient absent from the suppression list.

  The interim has a second, quieter dependency that is not about credentials: `infokit.org`'s registration **and** its Route 53 hosted zone both live in the old account, and the old account's SES identity is verified by Easy DKIM — meaning those three CNAME records in that zone are the verification mechanism, not merely the signing keys. SES re-checks them continuously, so the zone cannot be migrated while sending still depends on it; removing the records un-verifies the domain and stops sign-in mail. The registration can move independently of the zone — `route53domains transfer-domain-to-another-aws-account` does not disturb NS delegation — but not yet, and the reason is worth writing down so nobody re-attempts it and reads the refusal as a permissions problem. Route 53 will not transfer a domain within 14 days of its registration; `infokit.org` was registered 24 July 2026, so an attempt on 29 July returned `InvalidInput … The domain is not transferable at this time`, and the earliest workable date is 8 August 2026. It is a registration-age rule rather than a lock — `TransferLock` is null, the status is `ACTIVE`, and no operation is pending — and waiting costs nothing operationally: registration and hosted zone are independent, so mail, DNS and delegation behave identically whichever account holds the registration. The move is ownership hygiene, not a fix, since the domain is InfoKit's rather than the older project's. When it is made, the receiving account has three days to accept or the request is cancelled. The correct order for the zone is last: leave the sandbox, verify `infokit.org` in InfoKit's own account so a second set of CNAMEs exists, delete the `AWS_MESSAGING_*` variables, confirm delivery, and only then move DNS — at which point nothing depends on the old account and an error during the cutover costs nothing. The mechanism is deliberately two environment variables (`AWS_MESSAGING_ACCESS_KEY_ID`, `AWS_MESSAGING_SECRET_ACCESS_KEY`) that `apps/web/src/server/aws-credentials.ts` falls back from when unset, so the interim ends by deleting them once SES and SNS production access lands in InfoKit's account — no code change, which is what keeps a temporary arrangement from becoming structural. The borrowed credential must be a send-only IAM user; the old account's general-purpose `app-user` carries `AmazonS3FullAccess` and `AmazonDynamoDBFullAccess`, and lending those to a web deployment would put another project's data inside InfoKit's blast radius. The user created for this is `infokit-messaging` in `211125336353`, holding one inline policy named `infokit-send-only` (`ses:SendEmail`, `ses:SendRawEmail`, `sns:Publish`) and no managed policy — recorded here because its key pair lives only in a gitignored file, so this is the only committed trace that an identity in another account exists to be revoked. Revoking it is `aws iam delete-user-policy --user-name infokit-messaging --policy-name infokit-send-only`, which stops InfoKit sending without touching anything else in that account.

- `DECIDED (Slice 0 authentication, updated 21 July 2026)` — Each account has one normalized sign-in email and may authenticate by password or a single-use email link. Both methods create a revocable database session and require session-bound SMS verification before private access. Passwords use a salted memory-hard hash; sign-in is rate-limited and returns generic failure messages. AWS SES and SNS remain in `eu-west-3`. Credential delivery was revisited on 29 July 2026 as the first persistent deployment was prepared: local administration uses the `infokit` profile for storage and the `default` profile for sending, and the deployment carries explicit keys because Vercel has no `~/.aws` and no task role. The mobile authentication gate is still open.
