...
The mobile build now reads real data: `/api/public/activities` (list and detail) serves published content anonymously, `packages/api-client` types it, and the Expo screens render it — so the G0 gate ("faster than existing channels, on web **and** on the mobile build") can be answered for activities. Public payloads are prepared server-side by `apps/web/src/server/content/public-activity-payload.ts`, which the public web pages use too: add a field there rather than formatting a date or picking a translation on a client. Articles and the simulator have no public endpoint yet.
